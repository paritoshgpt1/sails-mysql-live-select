...
module.exports = (function() {
  ...
  var adapter = {
    ...
    multiUpdate: function(connectionName, collectionName, options, condition, tables, query ) {
      ...
      // Start live query, return LiveMysqlSelect object
      log('MYSQL.liveFind: ', condition);

      var table_list = [];
      tables.forEach(function(element){
        table_list.push({table: element});
      });

      var liveSelect = liveConnection.select(condition, table_list);

      return liveSelect;

    },

    liveFindWithPopulate: function(connectionName, collectionName, options, condition, query) {
      // Check if this is an aggregate query and that there is something to return
      if(options.groupBy || options.sum || options.average || options.min || options.max) {
        if(!options.sum && !options.average && !options.min && !options.max) {
          return cb(Errors.InvalidGroupBy);
      ...
      var liveSelect = liveConnection.select(condition, [ {
        table: schema[collectionName].tableName,
        condition: typeof condition === 'function' ? condition : undefined
      } ]);
